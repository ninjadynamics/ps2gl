#!/usr/bin/env python3
"""Reject unsafe generated road images before linking; never rewrite microcode.
...
def check(text):
    ...
    for raw in text.splitlines():
        ...
        if line == "vsmGeneralClipRoadX2R_CodeStart:":
            active = True
        if not active or not line:
            continue
        if line.endswith(":"):
            labels[line[:-1]] = len(code)
            if line == "vsmGeneralClipRoadX2R_CodeEnd:":
                break
        elif not line.startswith("."):
            code.append(line)
    if not code or "vsmGeneralClipRoadX2R_CodeEnd" not in labels:
        raise ValueError("missing road image boundaries")
...
def main():
    try:
        ...
        raise SystemExit(f"x2r microcode guard: FAIL: {error}")
    print(f"x2r microcode guard: PASS ({count} pairs, {padded} padded, {branches} bounded branches)")
