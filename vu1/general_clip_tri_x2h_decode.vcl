/* Exact-corner X2H decoder: X2C's paired wall colors with a fog value per
 * corner (height-dependent fog). Assembled separately from the unchanged X2
 * body; four descriptors expand to the same 24-vertex activation as X2C.
 ...
 *   117..128 COL: four descriptors x three qwords:
 *            [bottom RGB, -] [top RGB, -] [fogA fogB fogC fogD]
 ...
 * Geometry, UV and colors are bit copies (MOVE/MR32), not arithmetic: each
 * corner takes its row's RGB and its own EE-normalized fog. A=(bottom,fogA),
 * B=(bottom,fogB), C=(top,fogC), D=(top,fogD).
 */
kXcInput .equ 5
kXcGeo .equ 101
kXcCol .equ 117
kX2MainPc .equ 6
     ...
     .init_vf VF26-VF31
     .init_vi VI10-VI15
     .name vsmGeneralClipTriX2HDecode
     --enter
     --endenter
...
xh_uv_fence_lid:
     ...
     lq VF31, 2(VI14)       ; fogA fogB fogC fogD
     ...
     iaddiu VI13, VI13, 4
     iaddiu VI14, VI14, 3
     b xh_decode_loop_lid

xh_decode_done_lid:
     b xh_tail_fence_lid
xh_tail_fence_lid:
     iaddiu xh_jump, VI00, kX2MainPc
     --exit
     out_vi xh_jump (VI15)
     --endexit
     .END
