#!/usr/bin/env python3
"""Validate X2B image structure using the shared ground output contract."""
import sys
...
def check(text):
    symbol = "vsmGeneralClipBillboardX2B"
    for suffix in ("_CodeStart:", "_CodeEnd:"):
        if text.count(symbol + suffix) != 1:
            raise ValueError("missing or duplicate billboard image boundary")
...
def main():
    try:
        ...
        raise SystemExit(f"x2b microcode guard: FAIL: {error}")
    print(f"x2b microcode guard: PASS ({count} pairs, {padded} padded, "
